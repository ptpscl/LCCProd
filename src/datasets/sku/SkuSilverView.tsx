...
export default function SkuSilverView() {
  ...
  // ---- #4: per-member isolation state ----
  // Which members of the CURRENT family are checked "in" for this resolution.
  // Defaults to everyone checked; unchecking a member isolates it out of the
  // action being taken (e.g. 3 of 5 are real duplicates, 2 should be kept separate).
  ...
  useEffect(() => {
    if (currentFam) {
      setIncludedMembers(prev => {
        ...
        currentFam.members.forEach(m => { next[m.sku_code] = prev[m.sku_code] ?? true; });
        return next;
      });
    }
  }, [currentFam?.fid]);

  const includedList = currentFam ? currentFam.members.filter(m => includedMembers[m.sku_code]) : [];
  const isolatedList = currentFam ? currentFam.members.filter(m => !includedMembers[m.sku_code]) : [];

  const openReview = (def: TagDef) => {
    setOpenTag(def); setFamIdx(0); setStatusFilter('for_review');
    setChosenAction(def.resolutions[0].value); setAuditNote(''); setIncludedMembers({});
  };
  ...
  const completeResolution = () => {
    ...
    addResolution({
      ...
      audit_note: isolatedList.length > 0
        ? `${auditNote.trim()} (isolated: ${isolatedList.map(m => m.sku_code).join(', ')} kept separate)`
        : auditNote.trim(),
      resolved_by: currentUser?.email || 'demo user',
      resolved_at: new Date().toISOString(),
      // TODO(needs skuDemoData.ts): once DemoResolution supports it, pass the
      // isolated member codes as their own field instead of folding them into
      // the audit note, e.g. excluded_members: isolatedList.map(m => m.sku_code).
      // That's what lets SkuGoldView's isVerified()/roleOf() treat isolated
      // members as still-pending instead of resolved-by-association.
    });
    setResolutions(getResolutions());
    setAuditNote('');
    setToast({ message: `${currentFam.fid} resolved: ${opt.label}${isolatedList.length ? ` (${isolatedList.length} isolated)` : ''}`, type: 'success' });
              ...
              <>
                ...
                {!currentResolution && (
                  <p className="text-[12px] text-text-muted mb-2">
                    Uncheck any SKU that doesn't actually belong to this family — it'll be isolated out and kept separate from this resolution.
                  </p>
                )}
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-border-subtle bg-surface-bg">
                        {!currentResolution && <th className="px-4 py-2 text-[11px] font-semibold text-text-muted uppercase">Include</th>}
                    ...
                    <tbody className="divide-y divide-border-subtle">
                      {currentFam.members.map(m => {
                        const included = includedMembers[m.sku_code] ?? true;
                        return (
                          <tr key={m.sku_code} className={`hover:bg-surface-bg ${!included ? 'opacity-50 bg-surface-bg/60' : ''}`}>
                            {!currentResolution && (
                              <td className="px-4 py-2">
                                <input type="checkbox" checked={included}
                                  onChange={() => setIncludedMembers(prev => ({ ...prev, [m.sku_code]: !prev[m.sku_code] }))} />
                              </td>
                            )}
                            ...
                            <td className="px-4 py-2 whitespace-nowrap">
                              ...
                              {!included && <span className="ml-1 px-2 py-0.5 rounded-full bg-gray-100 text-gray-700 text-[11px] font-medium">Isolated</span>}
                            </td>
                          </tr>
                        );
                      })}
                ...
                {!currentResolution && isolatedList.length > 0 && (
                  <div className="mt-3 bg-amber-50 border border-amber-200 rounded-[8px] p-3">
                    <p className="text-[12px] text-amber-800">
                      <span className="font-semibold">{isolatedList.length} isolated:</span> {isolatedList.map(m => m.sku_code).join(', ')} — these will be noted as kept separate and won't be treated as part of this resolution.
                    </p>
                  </div>
                )}
              </>
