...
import { uploadSkuBatch } from './skuService';
import { validateSkuHeader } from './skuSchema';

interface SkuUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function SkuUploadModal({ isOpen, onClose, onSuccess }: SkuUploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        // Read the first chunk (2KB is enough for the 19-column header)
        const headerChunk = current.file.slice(0, 2048);
        const text = await headerChunk.text();
        const firstLine = text.split('\n')[0];

        const validation = validateSkuHeader(firstLine);
        if (!validation.ok) {
          const errMsgParts = [];
        ...
        await uploadSkuBatch(current.file, currentUser.email);
    ...
    <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-[12px] shadow-lg w-full max-w-[600px] overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between p-6 border-b border-border-subtle shrink-0">
          <h3 className="text-[18px] font-semibold text-text-main">Upload to SKU Hierarchy</h3>
                ...
                <div key={i} className="flex items-center justify-between bg-surface-bg border border-border-subtle p-3 rounded-[8px]">
                  ...
                  {f.status === 'pending' && (
                    <div className="flex items-center space-x-3 shrink-0">
                      <span className="text-[12px] text-text-muted mr-2">sku-hierarchy</span>
