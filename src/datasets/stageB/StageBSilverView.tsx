  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-[10px] border border-border-subtle bg-white shadow-subtle">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border-subtle px-6 py-5"><div><h3 className="text-[16px] font-semibold text-text-main">Inherited Dataset Anomaly Summary</h3><p className="mt-1 text-[12px] text-text-muted">Dataset-only lineage from Stage A and MMS. Counts stay separate from relational MATCH_STATUS and reviewed upstream anomalies do not require repeat resolution.</p></div><span className="inline-flex rounded-full bg-silver-bg px-2 py-1 text-[11px] font-medium text-silver-text">Lineage only</span></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[1050px] border-collapse text-left"><thead><tr className="border-b border-border-subtle bg-surface-bg">{['Source', 'DATASET_ONLY_ANOMALY', 'Affected Rows', 'Upstream Status', 'Definition'].map(label => <th key={label} className="px-6 py-3 text-[11px] font-semibold uppercase tracking-wider text-text-muted">{label}</th>)}</tr></thead><tbody className="divide-y divide-border-subtle">{INHERITED_DATASET_ANOMALIES.map(item => <tr key={`${item.source}-${item.code}`} className="hover:bg-surface-bg"><td className="px-6 py-4 text-[12px] font-medium text-text-main">{item.source}</td><td className="px-6 py-4 text-[12px] font-semibold text-text-main">{item.code}</td><td className="px-6 py-4 text-right text-[12px] text-text-main">{item.affectedRows.toLocaleString()}</td><td className="px-6 py-4"><span className={`rounded-full px-2 py-1 text-[11px] font-medium ${item.upstreamStatus === 'No anomalies' ? 'bg-gray-100 text-gray-800' : 'bg-green-100 text-green-800'}`}>{item.upstreamStatus}</span></td><td className="max-w-[420px] px-6 py-4 text-[12px] leading-5 text-text-muted">{item.definition}</td></tr>)}</tbody></table></div>
      </section>
