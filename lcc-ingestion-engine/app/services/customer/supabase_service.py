...
def download_file(path: str) -> bytes:
    try:
        return _client.storage.from_("bronze-raw").download(path)
    except Exception as exc:
        logger.exception("Failed to download customer file %s", path)
        raise RuntimeError(f"Could not download {path} from bronze-raw") from exc
